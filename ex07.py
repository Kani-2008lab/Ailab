...
scores.append(row)
returnnp.array(scores)

def predict(self, X: np.ndarray) ->np.ndarray:
        X = np.asarray(X, dtype=float)
jll = self._joint_log_likelihood(X)
idx = jll.argmax(axis=1)
returnself.classes_[idx]

defpredict_proba(self, X: np.ndarray) ->np.ndarray:
...
if alpha < 0:
raiseValueError("alpha must be >= 0")
self.alpha = alpha
self.classes_: np.ndarray | None = None
self.class_prior_log_: Dict[Any, float] = {}
...
def fit(self, X: np.ndarray, y: np.ndarray) -> "NaiveBayesMultinomial":
        X = np.asarray(X, dtype=float)
if (X < 0).any():
raiseValueError("Multinomial NB requires non-negative counts.")
        y = np.asarray(y)
self.classes_ = np.unique(y)
...
row = []
for c in self.classes_:
                # log P(c) + sum_ix_i * log P(feature_i | c)
...
vocab = ["buy", "discount", "meeting", "project"]
    ...
    X = np.array([
        ...
        [0, 0, 0, 3],  # "project projectproject"
        [1, 0, 1, 0],  # "buy meeting"
    ], dtype=float)
